...
stops<-read.csv(file = "/data/basic_reference_stops.txt", sep = '\t', header = TRUE, na.strings=c("","NA")) #transcript ID, chrom, start, end and gene symbol for all stops (stops with introns span 2 lines)
...
write.table(stops, file='/data/expanded_stop_seq.txt', quote=FALSE, sep='\t', col.names = FALSE, row.names = FALSE)
